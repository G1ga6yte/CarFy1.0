...
function ChatBlock (){
  ...
  useEffect(() => {
    ...
    const block3 = document.getElementById('appointBtn');
    const block4 = document.getElementById('appointBlock')
  ...
  function handleOfferDialog (){
    setOfferDialog(prev=>!prev)
  }
  
  return(
     <div className="ChatBlock G-container G-flex">
       <OfferDialog state={offerDialog} handleDialog={handleOfferDialog}/>
       <div className="headerBlock1 G-flex G-justify-between G-align-center">
         <div className="backBtn2" onClick={()=>{
           navigate(-1)
         }} style={{display: `${window.innerWidth < 992 ? tabletChat ? "none" : "flex" : "flex"}`}}>
           <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
             <path fillRule="evenodd" clipRule="evenodd" d="M15.7071 5.29289C16.0976 5.68342 16.0976 6.31658 15.7071 6.70711L10.4142 12L15.7071 17.2929C16.0976 17.6834 16.0976 18.3166 15.7071 18.7071C15.3166 19.0976 14.6834 19.0976 14.2929 18.7071L8.29289 12.7071C7.90237 12.3166 7.90237 11.6834 8.29289 11.2929L14.2929 5.29289C14.6834 4.90237 15.3166 4.90237 15.7071 5.29289Z" fill="#266EFE"/>
           </svg>
           {t("chat.back")}
         </div>
       ...
       <div style={{display: `${window.innerWidth <=992 ? tabletChat ? "block" : "none" : "block"}`}} className="chatCont">
          <div className="headerBlock G-flex G-justify-between">
            ...
            <button onClick={handleOfferDialog} id="appointBtn" style={{display: `${partner ? "block" : "none"}`}} className="newAppointmentBtn G-flex G-align-center">
              <img src={Images.appointment} alt=""/>
              {t("chat.btn2")}
              
              <div style={{display: `${appointBtn ? "block" : "none"}`, opacity: `${opc ? "1" : "0"}`}} id="appointBlock" className="textBlock">
                {t("chat.underText1")}
              </div>
            </button>
            <button style={{display: `${partner ? "none" : "block"}`}} disabled={true} className="enrollBtn">{t("chat.btn1")}</button>
          </div>
